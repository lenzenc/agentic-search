...
async def retrieve(state: AgentState) -> dict:
    ...
    collector_number_filter: str = state.get("detected_collector_number", "")
    set_total_filter: int | None = state.get("detected_set_total", None)
    ...
    if is_complex and sub_queries and iteration == 1:
        ...
        for sq in sub_queries:
            results = await search_for_query(
                sq["query"], INDEX_NAME, TOP_K, set_filter,
                collector_number_filter, set_total_filter
            )
            retrieved_ids = []
            for doc in results:
        ...
        results = await search_for_query(
            active_query, INDEX_NAME, TOP_K, set_filter,
            collector_number_filter, set_total_filter
        )
        for doc in results:
            card_id = doc.get("card_id", "")
